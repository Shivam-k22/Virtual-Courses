...
const allCategories = [
  ...
  "Database Management", "Python", "JavaScript", "Java", "C / C++", "DSA",
  "UI/UX Design", "Graphic Design", "Video Editing", "3D Modeling",
  "Digital Marketing", "Business & Entrepreneurship", "Finance & Accounting",
  "Project Management", "HR & Leadership", "Personal Development",
  "Language Learning", "Health & Fitness", "Photography", "Music",
  "App Development", "Data Analytics", "Ethical Hacking",
];
...
function AllCourses() {
  ...
  const clearAll = () => {
    setCategory([]); setLevel([]); setPrice("all"); setRating(0); setSearchQuery("");
  };

  const applyFilter = () => {
    let c = [...courseData];
    const q = searchQuery || location.state?.query;
    if (q) {
  ...
  useEffect(() => { applyFilter(); }, [courseData, category, level, price, rating, location.state, searchQuery]);

  const activeCount = category.length + level.length + (price !== "all" ? 1 : 0) + (rating > 0 ? 1 : 0) + (searchQuery ? 1 : 0);
  ...
  const renderSidebar = () => (
    <div className="flex flex-col h-full overflow-y-auto px-5 pb-8 [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-track]:bg-slate-100 [&::-webkit-scrollbar-thumb]:bg-blue-200 [&::-webkit-scrollbar-thumb]:rounded-full">
      ...
      <div className="relative mb-3">
        <svg className="absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-400" width="12" height="12" viewBox="0 0 16 16" fill="none">
        ...
        <input
          ...
          className="w-full pl-7 pr-3 py-1.5 text-[12px] rounded-lg border border-slate-200 bg-slate-50 text-slate-600 placeholder-slate-400 focus:outline-none focus:border-blue-300 focus:bg-white transition-colors"
        />
      </div>
      ...
      <div className="pr-1">
        {filteredCategories.length > 0 ? filteredCategories.map(cat => (
          ...
          <p className="text-[12px] text-slate-400 py-2">No categories found</p>
        )}
      </div>
  ...
  return (
    <div className="flex min-h-screen bg-slate-50 font-['DM_Sans',sans-serif]">
      ...
      {/* Desktop Sidebar */}
      <aside className="hidden lg:flex flex-col w-[280px] fixed top-0 left-0 h-full pt-[68px] bg-white border-r border-slate-100 shadow-sm">
        {renderSidebar()}
      </aside>
      ...
      {/* Main */}
      <main className="lg:ml-[280px] flex-1 pt-[68px] min-h-screen">

        {/* Top bar */}
        <div className="sticky top-[68px] z-10 bg-white/90 backdrop-blur-md border-b border-slate-100 shadow-sm">

          <div className="px-6 py-3.5 flex items-center gap-4 justify-between">
            {/* Title */}
            <div className="flex items-center gap-3 flex-shrink-0">
              ...
              <div>
                ...
                <p className="text-slate-400 text-[12px] font-medium mt-0.5">
                  ...
                  {(searchQuery || location.state?.query) && (
                    <span className="ml-1.5 text-blue-500">for "{searchQuery || location.state?.query}"</span>
                  )}
                  {location.state?.category && (
            ...
            <button
              ...
              className="lg:hidden flex items-center gap-2 px-3 py-2 rounded-lg border border-slate-200 text-slate-600 text-[13px] font-semibold hover:border-blue-300 hover:text-blue-600 transition-colors flex-shrink-0"
            >
          ...
          <div className="sm:hidden px-4 pb-3">
            <div className="relative">
              <svg className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" width="14" height="14" viewBox="0 0 16 16" fill="none">
                <circle cx="6.5" cy="6.5" r="5" stroke="currentColor" strokeWidth="1.5" />
                <path d="M10 10l4 4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
              </svg>
              <input
                ...
                className="w-full pl-9 pr-8 py-2 text-[13px] rounded-xl border border-slate-200 bg-white text-slate-700 placeholder-slate-400 focus:outline-none focus:border-blue-400 transition-all"
              />
              {searchQuery && (
                <button
        ...
        {activeCount > 0 && (
          <div className="px-6 py-2.5 flex flex-wrap gap-2 bg-blue-50 border-b border-blue-100">
            {searchQuery && (
              <span className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-blue-100 border border-blue-200 text-blue-700 text-[12px] font-semibold">
                🔍 "{searchQuery}"
                <button className="bg-transparent border-none cursor-pointer text-inherit p-0 text-[14px] leading-none opacity-70 hover:opacity-100" onClick={() => setSearchQuery("")}>×</button>
              </span>
            )}
            {category.map(c => (
              <span key={c} className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-blue-100 border border-blue-200 text-blue-700 text-[12px] font-semibold">
                {c}
                <button className="bg-transparent border-none cursor-pointer text-inherit p-0 text-[14px] leading-none opacity-70 hover:opacity-100" onClick={() => setCategory(p => p.filter(i => i !== c))}>×</button>
              </span>
            ))}
            {level.map(l => (
              <span key={l} className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-indigo-100 border border-indigo-200 text-indigo-700 text-[12px] font-semibold">
                {l}
                <button className="bg-transparent border-none cursor-pointer text-inherit p-0 text-[14px] leading-none opacity-70 hover:opacity-100" onClick={() => setLevel(p => p.filter(i => i !== l))}>×</button>
              </span>
            ))}
            {price !== "all" && (
              <span className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-100 border border-emerald-200 text-emerald-700 text-[12px] font-semibold">
                ...
                <button className="bg-transparent border-none cursor-pointer text-inherit p-0 text-[14px] leading-none opacity-70 hover:opacity-100" onClick={() => setPrice("all")}>×</button>
              </span>
            )}
            {rating > 0 && (
              <span className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-100 border border-amber-200 text-amber-700 text-[12px] font-semibold">
                {rating}+ stars
                <button className="bg-transparent border-none cursor-pointer text-inherit p-0 text-[14px] leading-none opacity-70 hover:opacity-100" onClick={() => setRating(0)}>×</button>
              </span>
            )}
          </div>
        )}
